...
**Data flow on target:** UART ISR → ring buffer → feed thread (wraps bytes in `net_buf` packets from a pool defined in `main.cpp`) → `RawHttpServer::enqueue_packet()` → k_fifo → the server's own thread → `http_parser_execute()` fires four callbacks (`on_url` accumulates, `on_headers_complete` opens the upload file, `on_body` streams fragments to disk zero-copy, `on_message_complete` *pauses the parser* — it must not answer inline, because the response is staged in the very packet the parser is reading) → the thread sees `HPE_PAUSED`, answers with the request's own `net_buf` reused for every output callback of the response, then unrefs it → `uart_poll_out()`. The class owns one thread and **no mutex**: the thread is the only toucher of request state, `enqueue_packet()` (ISR-safe) only touches the fifo, and single-consumer ordering serialises requests — a packet arriving mid-response waits in the fifo. Responses always carry `Content-Length` (never chunked). There is no connection object — keep-alive falls out of the design; a malformed request gets a 400 and a parser reset, losing nothing else. Bytes pipelined behind a complete request in the same packet are dropped (the buffer is being reused). Packet buffers must hold ≥ `RawHttpServer::response_head_max` (96) bytes. The ISR/feed-thread split exists because `net_buf_alloc(K_FOREVER)` may block on an exhausted pool, which the ISR may not; the ring absorbs bytes meanwhile. `uart_poll_out` blocking on the server thread is the TX flow control — the server can't outrun the wire.
...
- Magic numbers get a named constant with a one-line derivation, in the `.cpp` unless truly public API (see `response_head_max`).
