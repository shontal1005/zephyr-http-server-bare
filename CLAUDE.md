...
**Data flow on target:** UART ISR → ring buffer → feed thread → `RawHttpServer::input()` → `http_parser_execute()` fires four callbacks (`on_url` accumulates, `on_headers_complete` opens the upload file, `on_body` streams fragments to disk zero-copy, `on_message_complete` answers) → output callback → `uart_poll_out()`. Everything is synchronous inside `input()`; the class creates no threads and responses always carry `Content-Length` (never chunked). There is no connection object — keep-alive falls out of the design; a malformed request gets a 400 and a parser reset, losing nothing else. The ISR/feed-thread split exists because the ISR may not block and `input()` does file I/O; the ring absorbs bytes arriving during flash writes.
